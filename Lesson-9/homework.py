...
# 2.4 Если первые три курса "Python", "Kotlin", "JavaScript", то создайте срез, содержащий первые три курс
if course_list[:3] == ["Python", "Kotlin", "JavaScript"]:
    first_three_courses = course_list[:3]
...
#     # Циклы вы еще не проходили и для решения задачи эти знания не нужны. Просто пишите код с отступом, продолжая прог
...
if choice == "1":
    ...
    task_id = len(tasks) + 1
    task = [task_id, task_description, False]
    tasks.append(task)
    print(f"Задача '{task_description}' добавлена.")
elif choice == "2":
    if not tasks:
    ...
    task_id = int(input("Введите номер задачи, которую хотите удалить: "))
    if any(task[0] == task_id for task in tasks):
