...
# которая посчитает количество пробелов в ст
...
print(f"Количество четных чисел: {even_i}")
print(f"Количество нечетных чисел: {odd_i}")
...
for line in matrix:
    for i in line:
        if min(line) == i:
            print(f'Минимальное значение в ряду: {line} = {i}')
print('\n')
for line in matrix:
    for i in line:
        if max(line) == i:
...
#
# - Выведите исходные матрицы и результат их произведе