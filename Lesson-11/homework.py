...
for row in matrix:
    min_value = min(row)
    max_value = max(row)
    print(f"В строке {row}: Минимальное значение: {min_value}, Максимальное значение: {max_value}")
...
matrix1 = [[n + 1 + 3 * i for n in range(3)] for i in range(3)]
matrix2 = [[9 - n - 3 * i for n in range(3)] for i in range(3)]
