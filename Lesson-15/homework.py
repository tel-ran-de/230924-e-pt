### Тема: Рекурсия

# 1. Напишите функцию `sum_list(lst)`, которая возвращает сумму всех элементов списка `lst` с помощью рекурсии.
# Пример использования:
# print(sum_list([1, 2, 3, 4, 5]))  # Вывод: 15
def sum_list(lst):
    # Базовый случай: если список пустой, сумма равна 0
    if not lst:
        return 0
    else:
        # Рекурсивный случай: первый элемент + сумма оставшихся элементов
        return lst[0] + sum_list(lst[1:])

print(sum_list([1, 2, 3, 4, 5]))  # Вывод: 15


# 2. Напишите функцию `is_palindrome(s)`, которая проверяет, является ли строка `s` палиндромом
# (порядок букв одинаковый при чтении слева направо и справа налево) с помощью рекурсии.
# Пример использования:
# print(is_palindrome("radar"))  # Вывод: True
# print(is_palindrome("hello"))  # Вывод: False
# ОТВЕТ:
def is_palindrome(s):
    # Приводим строку к нижнему регистру и удаляем пробелы
    s = s.lower().replace(" ", "")

    # Базовый случай: если строка пустая или состоит из одного символа
    if len(s) <= 1:
        return True
    # Рекурсивный случай: сравниваем первый и последний символы
    elif s[0] == s[-1]:
        return is_palindrome(s[1:-1])  # Удаляем первый и последний символы и повторяем
    else:
        return False  # Если символы не равны, строка не палиндром

print(is_palindrome("radar"))  # Вывод: True
print(is_palindrome("hello"))  # Вывод: False
print(is_palindrome("A man a plan a canal Panama"))  # Вывод: True

# 3. Напишите функцию `find_max(lst)`, которая возвращает максимальный элемент в списке `lst` с помощью рекурсии.
# Пример использования:
# print(find_max([1, 5, 3, 9, 2]))  # Вывод: 9
def find_max(lst):
    # Базовый случай: если в списке только один элемент, он максимальный
    if len(lst) == 1:
        return lst[0]
    else:
        # Рекурсивный случай: находим максимальный элемент в подсписке
        max_of_rest = find_max(lst[1:])  # максимальный элемент оставшихся элементов
        return lst[0] if lst[0] > max_of_rest else max_of_rest  # сравниваем первый элемент с максимальным из подсписка

print(find_max([1, 5, 3, 9, 2]))  # Вывод: 9


# Тема: Дополнительная практика на рекурсию

# 1. Напишите функцию `sum_of_digits(n)`, которая возвращает сумму цифр числа `n` с помощью рекурсии.
# Пример использования:
# print(sum_of_digits(12345))  # Вывод: 15
# Ответ:
def sum_of_digits(n):
    # Базовый случай: если число равно 0, возвращаем 0
    if n == 0:
        return 0
    else:
        # Рекурсивный случай: последняя цифра + сумма цифр оставшейся части числа
        return n % 10 + sum_of_digits(n // 10)

print(sum_of_digits(12345))  # Вывод: 15


# 2. Напишите функцию `reverse_string(s)`, которая возвращает строку `s` в обратном порядке с помощью рекурсии.
# Пример использования:
# print(reverse_string("hello"))  # Вывод: "olleh"
#  ОТВЕТ:
def reverse_string(s):
    # Базовый случай: если строка пуста или содержит один символ, возвращаем её как есть
    if len(s) <= 1:
        return s
    else:
        # Рекурсивный случай: берем последний символ и добавляем к результату реверсированной оставшейся строки
        return s[-1] + reverse_string(s[:-1])

print(reverse_string("hello"))  # Вывод: "olleh"


# 3. Напишите функцию `list_length(lst)`, которая возвращает длину списка `lst` с помощью рекурсии.
# Пример использования:
# print(list_length([1, 2, 3, 4, 5]))  # Вывод: 5
#  ОТВЕТ:
def list_length(lst):
    # Базовый случай: если список пуст, длина равна 0
    if not lst:
        return 0
    else:
        # Рекурсивный случай: 1 (текущий элемент) + длина оставшейся части списка
        return 1 + list_length(lst[1:])

# Пример использования
print(list_length([1, 2, 3, 4, 5]))  # Вывод: 5


# Тема: Дополнительная практика на функции

# 1. Напишите функцию `multiply_all`, которая принимает произвольное количество числовых аргументов с помощью `*args`
# и возвращает их произведение.
# Пример использования:
# print(multiply_all(1, 2, 3, 4))  # Вывод: 24
# ОТВЕТ:
def multiply_all(*args):
    # Начальное значение для произведения
    product = 1

    # Перебираем все аргументы и умножаем их
    for num in args:
        product *= num
    return product

print(multiply_all(1, 2, 3, 4))  # Вывод: 24

# 2. Напишите функцию `merge_dicts`, которая принимает произвольное количество словарей с помощью `**kwargs`
# и возвращает один объединённый словарь.
# Пример использования:
# dict1 = {"a": 1, "b": 2}
# dict2 = {"c": 3, "d": 4}
# print(merge_dicts(**dict1, **dict2))  # Вывод: {'a': 1, 'b': 2, 'c': 3, 'd': 4}
# ОТВЕТ:
def merge_dicts(**kwargs):
    # Создаем пустой словарь для хранения объединённых результатов
    merged_dict = {}

    # Перебираем все переданные словари и обновляем объединённый словарь
    for dictionary in kwargs.values():
        merged_dict.update(dictionary)

    return merged_dict

dict1 = {"a": 1, "b": 2}
dict2 = {"c": 3, "d": 4}
print(merge_dicts(dict1=dict1, dict2=dict2))  # Вывод: {'a': 1, 'b': 2, 'c': 3, 'd': 4}

# 3. Напишите функцию `make_flatten`, которая создаёт функцию `flatten`, превращающую вложенный список в одноуровневый.
# Пример использования:
# flatten = make_flatten()
# print(flatten([1, [2, [3, 4], 5], 6]))  # Вывод: [1, 2, 3, 4, 5, 6]
# ОТВЕТ:
def make_flatten():
    def flatten(nested_list):
        result = []

        # Перебираем элементы вложенного списка
        for item in nested_list:
            if isinstance(item, list):
                # Если элемент - это список, рекурсивно вызываем flatten
                result.extend(flatten(item))
            else:
                # Иначе добавляем элемент в результат
                result.append(item)

        return result

    return flatten

flatten = make_flatten()
print(flatten([1, [2, [3, 4], 5], 6]))  # Вывод: [1, 2, 3, 4, 5, 6]

# 4. Напишите рекурсивную функцию `find_min`, которая возвращает минимальный элемент в списке `lst`.
# Пример использования:
# print(find_min([4, 2, 8, 1, 5]))  # Вывод: 1
# ОТВЕТ:
def find_min(lst):
    # Базовый случай: если список состоит из одного элемента, возвращаем его
    if len(lst) == 1:
        return lst[0]

    # Рекурсивный случай: ищем минимум в подсписке, исключая первый элемент
    min_of_rest = find_min(lst[1:])

    # Сравниваем первый элемент с минимальным элементом подсписка
    return min(lst[0], min_of_rest)

print(find_min([4, 2, 8, 1, 5]))  # Вывод: 1

# 5. Напишите функцию `show_info`, которая принимает произвольное количество именованных и неименованных аргументов
# с помощью `*args` и `**kwargs` и выводит их.
# Пример использования:
# args = (1, 2, 3)
# kwargs = {"name": "Alice", "age": 30}
# show_info(*args, **kwargs)
# Вывод:
# Args: (1, 2, 3)
# Kwargs: {'name': 'Alice', 'age': 30}
# ОТВЕТ:
def show_info(*args, **kwargs):
    # Вывод неименованных аргументов
    print("Args:", args)

    # Вывод именованных аргументов
    print("Kwargs:", kwargs)

args = (1, 2, 3)
kwargs = {"name": "Alice", "age": 30}
show_info(*args, **kwargs)
